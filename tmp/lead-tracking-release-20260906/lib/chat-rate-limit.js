...
const memoryState = globalThis.__tiresSosChatRateLimit || {
  entries: new Map(),
  operations: 0,
  warnedAboutRpc: false,
};
globalThis.__tiresSosChatRateLimit = memoryState;
...
export function getClientIp(request) {
  // Only trust headers set by the hosting proxy. On Vercel, x-vercel-forwarded-for
  // and x-forwarded-for are overwritten with the real client IP, so they cannot be
  // spoofed by the client. cf-connecting-ip / x-real-ip are NOT overwritten by
  // Vercel and are fully client-controllable, so they are trusted only when
  // TRUST_CF_CONNECTING_IP=1 is set (i.e. the app really sits behind Cloudflare).
  const trustCloudflare = process.env.TRUST_CF_CONNECTING_IP === "1";
  const cloudflare = trustCloudflare ? request.headers.get("cf-connecting-ip") : "";
  const vercel = request.headers.get("x-vercel-forwarded-for")?.split(",")[0];
  const forwarded = request.headers.get("x-forwarded-for")?.split(",")[0];
  const value = (cloudflare || vercel || forwarded || "unknown").trim().slice(0, 120);
  return value || "unknown";
}

// IP-scoped limiter for the admin password endpoint: blunts brute-forcing of the
// single shared ADMIN_PASSWORD beyond the fixed per-request delay.
export async function checkAdminLoginRateLimit(ip) {
  const max = boundedEnvInt("ADMIN_LOGIN_RATE_LIMIT_MAX", 10, 3, 100);
  const windowSeconds = boundedEnvInt("ADMIN_LOGIN_RATE_LIMIT_WINDOW_SECONDS", 900, 60, 86_400);
  const result = await consume("admin-login", ip, { limit: max, windowSeconds });
  return {
    ...result,
