...
export async function detectAnomalies(currentMetrics, historicalAverage, threshold = 0.3) {
  try {
    ...
    // CPC anomaly
    const cpcBaseline = Number(historicalAverage.avgCpc) || 0;
    const cpcDeviation = cpcBaseline > 0 ? Math.abs(currentMetrics.avgCpc - cpcBaseline) / cpcBaseline : 0;
    if (cpcBaseline > 0 && cpcDeviation > threshold) {
      anomalies.push({
        type: "CPC_SPIKE",
    ...
    // CTR anomaly
    const ctrBaseline = Number(historicalAverage.ctr) || 0;
    const ctrDeviation = ctrBaseline > 0 ? Math.abs(currentMetrics.ctr - ctrBaseline) / ctrBaseline : 0;
    if (ctrBaseline > 0 && ctrDeviation > threshold) {
      anomalies.push({
        type: "CTR_DROP",
