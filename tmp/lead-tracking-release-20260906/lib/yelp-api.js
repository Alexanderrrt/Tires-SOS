 ...
 */
export async function getYelpMetrics(connection, options = {}) {
  const manualSpend = Number(options?.manualSpend) || 0;
  if (!isConfigured(connection)) return unavailableMetrics("Yelp connection not configured");
  ...
  try {
    ...
    return { ...unavailableMetrics("Yelp Fusion API request failed", error.message), spend: manualSpend };
  }
}
