...
import { assistantAskedForName, detectWhatsAppService as detectService, detectWhatsAppServiceState as detectServiceState, isWhatsAppNameRefusal, isWhatsAppNonNameReply } from "./whatsapp-workflow";
...
export function recordsStoreConfigured() {
  return !isE2ETestMode() && Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY);
}
...
function cleanName(value) {
  ...
  if (isWhatsAppNameRefusal(value) || isWhatsAppNonNameReply(candidate)) return "";
  ...
  if (/\b(You|Can|Actually|Just|Call|Looking|Need|Want|Interested|Quote|Tire|Tires|Llanta|Llantas|Brakes|Frenos|Oil|Change|Aceite|Alignment|Alineacion|Battery|Bateria|Rim|Rims|Repair|Service|Appointment|Cita)\b/i.test(clean)) return "";
  return clean;
}

function extractName(_text, messages) {
  const patterns = [
    /\b(?:you can\s+)?call me\s+([a-zA-Z\u00c0-\u017f][a-zA-Z\u00c0-\u017f\s.'-]{1,40}?)(?=\b(?:and|but|phone|number)\b|[.!?,]|$)/i,
    /\b(?:puedes\s+)?(?:decirme|llamarme)\s+([a-zA-Z\u00c0-\u017f][a-zA-Z\u00c0-\u017f\s.'-]{1,40}?)(?=\b(?:y|pero|telefono|numero)\b|[.!?,]|$)/i,
    /\bdime\s+([a-zA-Z\u00c0-\u017f][a-zA-Z\u00c0-\u017f\s.'-]{1,40}?)(?=\b(?:y|pero|telefono|numero)\b|[.!?,]|$)/i,
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    const askedForName = previous?.role === "assistant" && assistantAskedForName(previousText);
...
export async function getAppointmentBySession(sessionId) {
  if (!recordsStoreConfigured()) {
...
export async function captureChatRecord({ sessionId, context = "shop", source = "", lang = "en", messages = [], assistantMessage = "" }) {
  const cleanMessages = sanitizeMessages(messages);
  const cleanSessionId = normalizeText(sessionId, 120);
  ...
  const serviceState = detectServiceState(cleanMessages);
  const extracted = {
    customerName: extractName(customerText, cleanMessages),
    phone: extractPhone(cleanMessages),
    vehicle: extractVehicle(cleanMessages),
  ...
  const fields = {
    ...
    tireSize: "",
    service: serviceState.rejected ? extracted.service : extracted.service || existingLead?.service || "",
  ...
  const lead = {
    ...
    source: normalizeText(source, 80) || existingLead?.source || (context === "quote" ? "Quote chat" : "Site chat"),
    lang: lang === "es" ? "es" : "en",
  ...
  return {
    captured: true,
    leadId: savedLead.id,
    leadComplete: Boolean(savedLead.customerName && savedLead.phone && savedLead.vehicle && savedLead.service),
...
export async function deleteRecord(type, id) {
  ...
  const { data: leadRow, error: leadReadError } = await db
    .from(LEADS_TABLE)
    .select("session_id")
    .eq("id", id)
    .maybeSingle();
  if (leadReadError) throw new Error(leadReadError.message);
  if (!leadRow) throw new Error("Record not found.");
  const [byLead, bySession] = await Promise.all([
    db.from(APPOINTMENTS_TABLE).select("id").eq("lead_id", id),
    db.from(APPOINTMENTS_TABLE).select("id").eq("session_id", leadRow.session_id),
  ]);
  if (byLead.error) throw new Error(byLead.error.message);
  if (bySession.error) throw new Error(bySession.error.message);
  const appointmentIds = [...new Set([...(byLead.data || []), ...(bySession.data || [])].map((row) => row.id))];
  if (appointmentIds.length) {
    const { error: appointmentDeleteError } = await db.from(APPOINTMENTS_TABLE).delete().in("id", appointmentIds);
    if (appointmentDeleteError) throw new Error(appointmentDeleteError.message);
  }
  ...
  return {
    ok: true,
    persisted: true,
    deleted: { type, id, appointmentIds, leadIds: [id] },
  };
}
...
export async function createManualAppointment({
  ...
  const lead = {
    id: stableId("lead"),
    sessionId,
    source: normalizeText(source, 40) || "Manual",
    lang: lang === "es" ? "es" : "en",
    status: "new",
    createdAt: timestamp,
    updatedAt: timestamp,
