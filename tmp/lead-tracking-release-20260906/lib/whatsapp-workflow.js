...
export function hasWhatsAppCancellationIntent(value) {
  return /\b(cancel(?:ar|a|e)?|anular|remove|delete|elimin(?:ar|a|e)|borrar?)\b.{0,40}\b(appointment|booking|reservation|cita|reserva)\b/i.test(foldedText(value))
    || /\b(appointment|booking|reservation|cita|reserva)\b.{0,40}\b(cancel(?:ar|a|e)?|anular|remove|delete|elimin(?:ar|a|e)|borrar?)\b/i.test(foldedText(value));
}

export function assistantAskedForName(value) {
  const text = foldedText(value);
  return /(?:what(?:'?s| is) your name|your name\??|what name should i (?:put|use)|cual es tu nombre|como te llamas|a nombre de quien)/i.test(text);
}
...
export function withWhatsAppWelcome(reply, lang) {
  const body = String(reply || "").trim();
  if (!body || /thanks for contacting Tires SOS Rescue|gracias por contactar a Tires SOS Rescue/i.test(body)) return body;
  const welcome = lang === "es"
    ? "¡Hola! Gracias por contactar a Tires SOS Rescue."
    : "Hi! Thanks for contacting Tires SOS Rescue.";
  return `${welcome}\n\n${body}`;
}
...
export function detectWhatsAppHandoff(history = [], { appointmentConfirmed = false } = {}) {
  ...
  if (/\b(?:no me entiendes|no entiende|no ayudas|no sirve|sigues preguntando|misma pregunta|preguntas otra vez|por que.{0,24}otra vez|this is not helping|you do not understand|you dont understand|same question|you keep asking|why are you asking|asking again)\b/i.test(latest)) {
