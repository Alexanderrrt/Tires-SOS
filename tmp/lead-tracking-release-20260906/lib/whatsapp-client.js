...
export function isExpectedWhatsAppWebhookRecipient(metadata, configuredPhoneNumberId = process.env.WHATSAPP_PHONE_NUMBER_ID) {
  const expected = String(configuredPhoneNumberId || "").trim();
  const received = String(metadata?.phone_number_id || "").trim();
