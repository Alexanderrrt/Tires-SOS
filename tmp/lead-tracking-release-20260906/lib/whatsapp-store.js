...
export async function saveInboundWhatsAppMessage({ messageId, waId, customerName, body }) {
  const client = db();
  const fields = "id,wa_id,customer_name,bot_enabled,context_enabled,offered_slots,lead_session_id,appointment_id";
  const { data: existing, error: readError } = await client.from("whatsapp_conversations").select(fields).eq("wa_id", waId).maybeSingle();
  if (readError) throw readError;
  const timestamp = new Date().toISOString();
  const query = existing
    ? client.from("whatsapp_conversations").update({
      ...(customerName ? { customer_name: customerName } : {}),
      last_message_at: timestamp,
      updated_at: timestamp,
    }).eq("id", existing.id)
    : client.from("whatsapp_conversations").insert({
      wa_id: waId,
  ...
  const { data: conversation, error: conversationError } = await query.select(fields).single();
  if (conversationError) throw conversationError;
  const { data: insertedMessages, error } = await client.from("whatsapp_messages").upsert({
  ...
  return { ...conversation, inboundMessageSaved: Boolean(insertedMessages?.length) };
}
...
export async function getWhatsAppResetState(id) {
  const { data, error } = await db().from("whatsapp_conversations").select("lead_session_id").eq("id", id).single();
  if (error) throw error;
  return { leadSessionId: data?.lead_session_id || null };
}

export async function setWhatsAppBookingState(id, { offeredSlots = [], leadSessionId, appointmentId }) {
