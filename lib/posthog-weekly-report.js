...
function reportWindow(now = new Date()) {
  ...
  const periodEnd = new Date(endExclusive.getTime() - 86400000);
  return {
...
function buildHtml(data) {
  const { periodStart, periodEnd, summary, prior, topPages, contactLocations, quoteClasses, appointments, funnel, highlights, observedEvents } = data;
  const cards = [
...
export async function generateWeeklyAnalyticsReport(now = new Date()) {
