...
function plainSummary(summary, prior, trafficSources) {
  ...
  const trendEn = change > 5 ? `up ${Math.abs(change).toFixed(0)}% from last time` : change < -5 ? `down ${Math.abs(change).toFixed(0)}% from last time` : "about the same as last time";
  const trendEs = change > 5 ? `${Math.abs(change).toFixed(0)}% más que la vez pasada` : change < -5 ? `${Math.abs(change).toFixed(0)}% menos que la vez pasada` : "similar a la vez pasada";
  const top = trafficSources.find((s) => s.label !== "Direct") || trafficSources[0];
  ...
  const leadsEn = leads > 0
    ? `${leads} customer${leads === 1 ? "" : "s"} sent a quote request or booked an appointment — follow up soon!`
    : "No quote requests or appointments came in yet — consider promoting your booking link.";
  const leadsEs = leads > 0
    ? `${leads} cliente${leads === 1 ? "" : "s"} envió una cotización o agendó una cita — ¡síguelos pronto!`
    : "Todavía no llegaron cotizaciones ni citas — considera promocionar tu enlace de reservas.";
  return bi(
...
export async function generateWeeklyAnalyticsReport(range = {}) {
  ...
  const highlights = metrics.filter(([, current, old]) => Math.abs(percentChange(current, old)) > 30).map(([label, current, old]) => {
    ...
    return bi(
      `${label.en} ${up ? "jumped" : "dropped"} to ${current.toLocaleString()} (from ${old.toLocaleString()} last time).`,
      `${label.es} ${up ? "subió" : "bajó"} a ${current.toLocaleString()} (antes ${old.toLocaleString()}).`
    );
  });
  const data = {
    ...window,
