...
function aggregateCampaigns(rows, period) {
  ...
  rows.filter((row) => row.event === "$pageview" && row.period === period && row.utm_source).forEach((row) => {
    const key = [row.utm_source, row.utm_medium, row.utm_campaign, row.utm_content, row.utm_term]
      .filter(Boolean)
      .join(" / ");
...
export async function generateWeeklyAnalyticsReport(range = {}) {
  ...
  const dimensions = await queryPostHog(`
    select
      ...
      coalesce(nullIf(toString(properties.utm_medium), ''), '') as utm_medium,
      coalesce(nullIf(toString(properties.utm_campaign), ''), '') as utm_campaign,
      coalesce(nullIf(toString(properties.utm_content), ''), '') as utm_content,
      coalesce(nullIf(toString(properties.utm_term), ''), '') as utm_term,
    ...
    group by period, event, location, vehicle_class, persisted, page_path, referring_domain, utm_source, utm_medium, utm_campaign, utm_content, utm_term
