...
function buildManualReplyHtml(replyText, store) {
  ...
  return renderBrandedEmail({
    ...
    footerNote: `Manual reply to your Yelp Request a Quote for ${store.full}.`,
    location: store,
  });
}
...
export async function sendManualYelpReply({ gmailMessageId, replyText }) {
  ...
  const store = getStoreContact(inspected.storeId);
  const result = await sendGmailEmail({
    ...
    html: buildManualReplyHtml(safeReplyText, store),
    text: safeReplyText,
