...
- NEVER state a specific price or price range — you don't have access to real-time pricing here. Invite them to call, WhatsApp, or visit for an exact quote.
- Keep it short: 2-4 sentences, plain text (no markdown, no bullet points, no subject line, no HTML).
- End with a light, natural invitation to reach out (phone/WhatsApp) or stop by, using the business facts below. Do not repeat every business fact — pick what's relevant.
- Never invent business facts beyond what's given below.
- Do not sign off with a generic "Customer Service Team" — sign as "Tires SOS Rescue".
...
export async function generateYelpLeadReply({ customerMessage, customerName }) {
  ...
  const result = await callGroqChat(
    ...
    { maxTokens: 300, temperature: 0.4, timeoutMs: 15_000 },
  );
