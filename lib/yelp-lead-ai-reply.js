...
import { getPublicSite } from "./location-config";

function buildBusinessFacts(site) {
  return [
    `Phone / WhatsApp: ${site.phone}`,
    `Locations: ${site.locations.map((location) => location.status === "mystery" ? "Hayward, CA (coming soon)" : location.full).join("; ")}`,
    `Hours: ${SITE.hours
...
export async function generateYelpLeadReply({ customerMessage, customerName }) {
  ...
  const businessFacts = buildBusinessFacts(await getPublicSite());
