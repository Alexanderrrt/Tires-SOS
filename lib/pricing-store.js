...
const REMOVED_SERVICES = new Set(["used-tires"]);

function mergeNewDefaults(stored) {
  const pricing = withoutPrivateSettings(stored);
  const cleanedServices = pricing.services.filter((s) => !REMOVED_SERVICES.has(s.id));
  const have = new Set(cleanedServices.map((s) => s.id));
  const missingServices = DEFAULT_PRICING.services.filter((s) => !have.has(s.id));
  const merged = { ...pricing, services: [...cleanedServices, ...missingServices] };
  // Older stored documents predate brand tiers / tire brands — add the
  // defaults so the admin UI and quote engine have something to work with.
  let result = merged;
  if (!Array.isArray(result.brandTiers) || !result.brandTiers.length) {
    result = { ...result, brandTiers: DEFAULT_PRICING.brandTiers };
  }
  if (!Array.isArray(result.tireBrands) || !result.tireBrands.length) {
    result = { ...result, tireBrands: DEFAULT_PRICING.tireBrands };
  }
  return result;
}
