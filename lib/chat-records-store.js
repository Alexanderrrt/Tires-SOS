...
function detectTiming(text) {
  const folded = foldedText(text);
  const match =
    folded.match(/\b(today|tomorrow|this morning|this afternoon|tonight|asap|now|morning|afternoon|evening)\b/i) ||
    folded.match(/\b(hoy|manana|esta tarde|esta manana|ahora|urgente)\b/i) ||
    folded.match(/\b(?:mon|tue|wed|thu|fri|sat|sun)(?:day)?\b/i) ||
    folded.match(/\b\d{1,2}(?::\d{2})?\s?(?:am|pm)\b/i) ||
    folded.match(/\b\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?\b/);
