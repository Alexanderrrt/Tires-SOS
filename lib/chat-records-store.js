...
function shouldCaptureBookingLead(messages, fields, customerText) {
  ...
  if (fields?.customerName && fields?.phone && fields?.vehicle && (fields?.service || fields?.preferredDate || fields?.preferredTime || fields?.preferredTimeText)) {
    return true;
  }
  ...
  return Boolean(fields?.customerName && fields?.phone && fields?.vehicle && readyWords);
}
