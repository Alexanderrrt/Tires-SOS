...
export async function captureChatRecord({ sessionId, context = "shop", lang = "en", messages = [], assistantMessage = "" }) {
  ...
  const hasContactInfo = Boolean(lead.phone || lead.customerName);
  const hadContactInfo = Boolean(existingLead?.phone || existingLead?.customerName);
  if (hasContactInfo && !hadContactInfo) {
    try {
      await notifyLead({
