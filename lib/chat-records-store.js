...
function cleanName(value) {
  ...
  if (/\b(Looking|Need|Want|Interested|Quote|Tire|Tires|Llanta|Llantas|Brakes|Frenos)\b/i.test(clean)) return "";
  return clean;
}
...
function extractName(_text, messages) {
  ...
  for (const message of [...userMessages(messages)].reverse()) {
    ...
    const raw = (lastSep !== -1 ? beforePhone.slice(lastSep + 1) : (beforePhone.split(/[\s]+/).pop() || "")).trim();
...
function shouldCaptureBookingLead(messages, fields, customerText) {
  if (hasLeadSignal(customerText)) return true;
  if (fields?.customerName && fields?.phone && (fields?.service || fields?.vehicle || fields?.preferredDate || fields?.preferredTime || fields?.preferredTimeText)) {
    return true;
  }
  ...
  return Boolean(fields?.customerName && fields?.phone && readyWords);
}
...
export async function captureChatRecord({ sessionId, context = "shop", lang = "en", messages = [], assistantMessage = "" }) {
  ...
  const fields = {
    ...
    tireSize: extracted.tireSize || existingLead?.tireSize || "",
    service: extracted.service || existingLead?.service || "",
    preferredDate: extracted.preferredDate || existingLead?.preferredDate || "",
    preferredTime: extracted.preferredTime || (validTimeString(existingLead?.preferredTime) ? existingLead.preferredTime : ""),
    preferredTimeText: extracted.preferredTimeText || existingLead?.preferredTimeText || "",
  };
  if (!shouldCaptureBookingLead(cleanMessages, fields, customerText)) return { captured: false };
  const appointmentRequested = Boolean(
    existingLead?.appointmentRequested
      ...
      || fields.preferredTimeText
      || (fields.customerName && fields.phone && (fields.service || fields.vehicle)),
  );
  const lead = {
  ...
  if (appointmentRequested) {
    ...
    lead.preferredDate = lead.preferredDate || preference.preferredDate || "";
    lead.preferredTime = lead.preferredTime || preference.preferredTime || "";
    lead.preferredTimeText = lead.preferredTimeText || preference.preferredTimeText || "";
  }
  ...
  if (appointmentRequested) {
    ...
    const appointment = {
      id: existingAppointment?.id || stableId("appt", cleanSessionId),
      leadId: savedLead.id,
      sessionId: cleanSessionId,
...
export async function reserveAppointment(identifier, scheduledDate, scheduledTime) {
  ...
  if (!recordsStoreConfigured()) {
    ...
    if (!lead.customerName || !lead.phone) {
      throw Object.assign(new Error("Name and phone are required before reserving."), { code: "CHAT_RESERVATION_CONTACT_REQUIRED" });
    }
    lead.appointmentRequested = true;
  ...
  if (leadId || sessionId) {
    ...
    const query = client().from(LEADS_TABLE).update(updatePayload);
    if (leadId) query.eq("id", leadId);
    else query.eq("session_id", sessionId);
    await query.select("id").catch(() => null);
  }
...
export async function createManualAppointment({
  ...
  const appointment = {
    ...
    preferredTime: time,
    preferredTimeText: "",
    notes: normalizeText(notes, 6000),
    scheduledDate: date,
    scheduledTime: time,
  };
