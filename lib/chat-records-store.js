...
function hasLeadSignal(text, context) {
  return (
    ...
    Boolean(extractPhoneFromText(text)) ||
    Boolean(text.match(/\b(?:P|LT)?\d{3}\/\d{2}(?:[Rr]|\/)\d{2}\b/)) ||
    Boolean(detectServiceInText(text))
  );
}
...
export async function captureChatRecord({ sessionId, context = "shop", lang = "en", messages = [], assistantMessage = "" }) {
  ...
  const appointmentRequested = Boolean(
    existingLead?.appointmentRequested || wantsAppointment(customerText) || fields.preferredDate || fields.preferredTime || fields.preferredTimeText,
  );
  const lead = {
