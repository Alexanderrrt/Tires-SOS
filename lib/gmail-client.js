...
function parseRetryAfterMs(response) {
  ...
  return Number.isFinite(dateMs) ? Math.max(1_000, dateMs - Date.now()) : 0;
}
...
async function gmailFetch(path, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    if (idempotent && Date.now() < gmailCooldownUntil) {
      await sleep(gmailCooldownUntil - Date.now());
    }
    ...
    if ((rateLimited || response.status >= 500) && attempt < maxAttempts) {
      const headerDelay = parseRetryAfterMs(response);
      ...
      if (rateLimited) gmailCooldownUntil = Math.max(gmailCooldownUntil, Date.now() + delayMs);
      await sleep(delayMs);
      continue;
    }
    const finalRetryAfterMs = rateLimited
      ? Math.max(parseRetryAfterMs(response), gmailCooldownUntil - Date.now(), 1_000)
      : 0;
