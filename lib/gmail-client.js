...
import { getGmailCooldownUntil, setGmailCooldownUntil } from "./yelp-leads-store.js";
import { EMAIL_LOGO_CID, EMAIL_LOGO_URL } from "./email-branding.js";
import { buildEmailMime } from "./email-mime.js";
...
const YELP_REJECTION_QUERY = 'from:yelp.com subject:"RE:" is:unread newer_than:30d';
...
export function isYelpRelayRejectionEmail(email) {
  const content = `${email?.subject || ""}\n${email?.text || ""}\n${email?.html || ""}`
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ");
  return /sorry,? we were not able to process your reply|email client that we do not yet support/i.test(content);
}

/** Lists unread Yelp delivery failures without treating them as customer leads. */
export async function listUnreadYelpRejectionEmails() {
  const search = await gmailFetch(`/messages?q=${encodeURIComponent(YELP_REJECTION_QUERY)}&maxResults=25`);
  const emails = [];
  for (const { id } of search.messages || []) {
    const email = await getGmailMessage(id);
    if (isYelpRelayRejectionEmail(email)) emails.push(email);
  }
  return emails;
}

/** Finds the original Yelp relay message within a rejection notice's thread. */
export async function findYelpRelaySourceMessageId(threadId, rejectionMessageId) {
  const id = safeGmailId(threadId, "thread ID");
  const rejectionId = safeGmailId(rejectionMessageId, "message ID");
  const thread = await gmailFetch(`/threads/${id}?format=full`);
  const source = (thread.messages || [])
    .map(messageFromApi)
    .find(
      (message) =>
        message.gmailMessageId !== rejectionId &&
        !(message.labelIds || []).includes("SENT") &&
        /@messaging\.yelp\.com\b/i.test(message.from),
    );
  return source?.gmailMessageId || null;
}
...
async function loadEmailLogo() {
  if (!emailLogoPromise) {
    emailLogoPromise = (async () => {
      const response = await fetch(EMAIL_LOGO_URL, {
        headers: { Accept: "image/png" },
        cache: "force-cache",
        signal: AbortSignal.timeout(6_000),
      });
      if (!response.ok) throw new Error(`Email logo request failed (${response.status}).`);
      const contentType = response.headers.get("content-type") || "";
      const content = Buffer.from(await response.arrayBuffer());
      if (!contentType.startsWith("image/png") || !content.length || content.length > 100_000) {
        throw new Error("Email logo response was invalid.");
      }
      return {
        cid: EMAIL_LOGO_CID,
      ...
      console.warn("Inline email logo unavailable; using the hosted image instead.", error?.message || error);
      return null;
    });
  }
 ...
 */
export async function sendGmailEmail({ to, fromEmail, fromName, subject, html, text, replyTo, threadId, inReplyToMessageId }) {
  const wantsLogo = String(html || "").includes(`cid:${EMAIL_LOGO_CID}`);
  const logo = wantsLogo ? await loadEmailLogo() : null;
  const safeHtml = wantsLogo && !logo
    ? String(html).replaceAll(`cid:${EMAIL_LOGO_CID}`, EMAIL_LOGO_URL)
    : html;
