...
/** Marks a provider-rejected reply failed while retaining its drafted copy. */
export async function markYelpLeadDeliveryFailed(id) {
  const { data, error } = await supabase()
    .from("yelp_leads")
    .update({ status: "failed", replied_at: null })
    .eq("id", id)
    .select("id")
    .single();
  if (error) throw error;
