...
function serviceLine(service, currency, laborRate) {
  ...
  if (service?.chatQuotable === false) {
    return `- id="${service.id}" ${name}: price varies too much to quote in chat (depends on exact type/specs and vehicle). Do NOT call the tool for this id. If asked, say the price varies and the shop will confirm it in person or by phone.`;
  }
...
export function compactPricingContext(pricing) {
  ...
  const header = [
    ...
    `Currency: ${currency}.`,
    vehicles ? `vehicleClass ids: ${vehicles}. Match by body style (small→compact, everyday→sedan, SUV/truck→suv_truck, luxury/performance→luxury_perf); default sedan if unclear.` : "No vehicle classes configured.",
