...
// Development-only migration fallback. Production must always provide an
// explicit NOTIFY_SMS_RECIPIENT so customer details cannot be silently routed
// to a number inherited from the source tree.
const DEVELOPMENT_SMS_RECIPIENT = "9834564801@vtext.com";

const REQUEST_TIMEOUT_MS = 3000;
const MAX_ATTEMPTS = 3;
const RETRY_BASE_DELAY_MS = 200;
const SMS_GATEWAY_RECIPIENT = /^\d{10,15}@[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?\.[a-z]{2,}$/i;
...
function validRecipient(value) {
  return SMS_GATEWAY_RECIPIENT.test(value);
}
...
function notificationConfig() {
  ...
  const configuredRecipient = envValue("NOTIFY_SMS_RECIPIENT");
  if (configuredRecipient) {
    return validRecipient(configuredRecipient) ? { ...provider, recipient: configuredRecipient } : null;
  }

  if (process.env.NODE_ENV === "production") return null;
  return { ...provider, recipient: DEVELOPMENT_SMS_RECIPIENT };
}
