...
export async function deliverLeadNotification(identifier) {
  ...
  if (existing.appointmentRequested && (!existing.preferredDate || !existing.preferredTime)) {
    return publicState(existing, "awaiting_time");
  }
  ...
  try {
    const result = await notifyLead({
      type: lead.appointmentRequested ? "APPOINTMENT" : lead.source === "Quote chat" ? "QUOTE LEAD" : "CHAT LEAD",
