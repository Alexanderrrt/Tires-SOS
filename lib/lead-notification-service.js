...
import { detectStoreContact } from "./store-contact.js";
...
export async function deliverLeadNotification(identifier) {
  ...
  const store = detectStoreContact({
    text: [
      lead.source,
      lead.summary,
      ...(Array.isArray(lead.transcript)
        ? lead.transcript.filter((message) => message?.role === "user").map((message) => message?.content)
        : []),
    ].filter(Boolean).join(" "),
  });
  try {
