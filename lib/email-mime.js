...
export function buildEmailMime({
  ...
  // Callers that intentionally omit HTML receive a genuine text-only message.
  // Relay services with stricter client compatibility requirements should
  // provide a minimal HTML equivalent and use multipart/alternative instead.
  if (!html) {
    return [
      ...headers,
      'Content-Type: text/plain; charset="UTF-8"',
