  ...
  try {
    const result = await sendReportEmail({
      ...
      html: renderBrandedEmail({
        ...
        primary: { href: "https://tiressosrescue.com/dashboard", label: "Open dashboard" },
...
export async function sendDailySummary(summaryData) {
  ...
  const html = renderBrandedEmail({ eyebrow: "Daily marketing report", title: "Daily ad performance", intro: `Summary for ${summaryData.date}.`, content, primary: { href: "https://tiressosrescue.com/dashboard", label: "Open dashboard" }, footerNote: "Automated performance insights for Tires SOS Rescue." });
...
export async function sendBudgetAlert(threshold, currentSpend) {
  ...
  const html = renderBrandedEmail({ eyebrow: "Budget alert", title: "Your ad budget needs attention", intro: "Review your campaigns before the monthly limit is reached.", content, primary: { href: "https://tiressosrescue.com/dashboard", label: "Review campaigns" }, footerNote: "Automated alert from Tires SOS Rescue." });
