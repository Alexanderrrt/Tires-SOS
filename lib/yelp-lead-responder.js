...
function fallbackReply(store) {
  return "Thanks so much for reaching out through Yelp! We received your request and a member of our team will " +
    `follow up with you shortly. If it's urgent, call us at ${store.phone} or stop by ${store.full} — ${store.businessName}.`;
}
...
function buildReplyHtml(replyText, store) {
  ...
  return renderBrandedEmail({
    preheader: `Thanks for reaching out to ${store.businessName}`,
    eyebrow: store.businessName,
    title: "We got your request!",
    content: paragraphs,
    primary: { href: store.whatsappHref, label: "WhatsApp us" },
    secondary: { href: store.phoneHref, label: "Call us" },
    footerNote: `Replying to your Yelp Request a Quote for ${store.full}.`,
    location: store,
  });
}
...
async function processLead(rawEmail) {
  ...
  try {
    await sendGmailEmail({
      ...
      html: buildReplyHtml(replyText, store),
      text: replyText,
