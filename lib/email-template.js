...
import { EMAIL_LOGO_CID } from "./email-branding.js";
import { getStoreContact } from "./store-contact.js";

const ORIGIN = SITE.url.replace(/\/$/, "");
const EMAIL_LOGO = `cid:${EMAIL_LOGO_CID}`;
...
export function renderBrandedEmail({ preheader, eyebrow, title, intro, content, primary, secondary, footerNote, location }) {
  const contact = location || getStoreContact("taylor");
  return `<!doctype html>
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:640px;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 8px 28px rgba(24,34,48,.10);">
        ...
        <tr><td style="background:#182230;padding:24px 30px;">
          <p style="color:#ffffff;font-size:14px;font-weight:700;margin:0 0 8px;">${escapeHtml(contact.businessName || "Tires SOS Rescue")}</p>
          <p style="color:#dbe2e9;font-size:12px;line-height:19px;margin:0 0 8px;">${escapeHtml(contact.full || "")}${contact.phone ? `<br>${escapeHtml(contact.phone)}` : ""}</p>
          <p style="color:#b9c3ce;font-size:12px;line-height:19px;margin:0 0 15px;">${escapeHtml(footerNote || "Servicio rápido y amable de llantas y autos en San José.")}</p>
          <p style="margin:0;">${button(contact.whatsappHref || SITE.whatsappHref, "WhatsApp", true)}${button(contact.phoneHref || SITE.smsHref, "Llamar", false)}</p>
          <p style="font-size:11px;line-height:18px;margin:9px 0 0;color:#8f9baa;"><a href="${contact.mapsHref || SITE.locations[0].mapsHref}" style="color:#fbbf24;text-decoration:none;">Cómo llegar</a> &nbsp;·&nbsp; <a href="${ORIGIN}" style="color:#fbbf24;text-decoration:none;">Visitar sitio web</a></p>
        </td></tr>
      </table>
      <p style="font-size:11px;line-height:17px;color:#8f9baa;margin:16px 12px 0;text-align:center;">Este mensaje fue enviado por el sitio web de Tires SOS Rescue.</p>
    </td></tr>
  </table>
</body></html>`;
}
