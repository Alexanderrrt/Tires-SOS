...
export function ServicesHubView() {
  ...
  return <main className="seo-page"><div className="seo-page__inner"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">{t(UI.home)}</Link><span>/</span><span>{t(UI.services)}</span></nav><header className="seo-hero"><p className="seo-eyebrow">Tires SOS Rescue</p><h1>{t(UI.serviceTitle)}</h1><p>{t(UI.serviceIntro)}</p></header><section className="seo-card-grid" aria-label={t(UI.services)}>{Object.entries(SERVICE_PAGES).map(([slug, service]) => { const item = localizedService(slug, service, lang); return <article className="seo-card" key={slug}><h2><Link href={`/services/${slug}`}>{item.name}</Link></h2><p>{item.description}</p><Link className="seo-text-link" href={`/services/${slug}`}>{t(UI.learn)} {item.name.toLowerCase()} →</Link></article>; })}</section><section className="seo-cta"><h2>{t(UI.unsure)}</h2><p>{t(UI.unsureBody)}</p><Link className="btn btn--primary" href="/quote">{t(UI.quote)}</Link></section></div></main>;
}

export function ServiceLandingView({ slug, service }) {
  const { lang, t } = useCopy(); const item = localizedService(slug, service, lang);
  return <main className="seo-page"><div className="seo-page__inner"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">{t(UI.home)}</Link><span>/</span><Link href="/services">{t(UI.services)}</Link><span>/</span><span>{item.name}</span></nav><header className="seo-hero"><p className="seo-eyebrow">{t(UI.sanJoseCare)}</p><h1>{lang === "es" ? `${item.name} en San José, CA` : service.title}</h1><p>{item.intro}</p><div className="seo-actions"><Link className="btn btn--primary" href="/quote">{t(UI.quote)}</Link><a className="btn btn--ghost" href={SITE.whatsappHref} target="_blank" rel="noopener noreferrer">{t(UI.whatsapp)}</a></div></header><section className="seo-split"><div><h2>{t(UI.expect)}</h2><ul className="seo-checklist">{item.benefits.map((benefit) => <li key={benefit}>{benefit}</li>)}</ul></div><aside><h2>{t(UI.twoShops)}</h2><p>{t(UI.twoShopsBody)}</p><Link className="seo-text-link" href="/locations">{t(UI.viewLocations)} →</Link></aside></section><section className="seo-faq"><h2>{item.name}: {t(UI.faqs)}</h2>{item.faq.map(([q, a]) => <details key={q}><summary>{q}</summary><p>{a}</p></details>)}</section><section className="seo-related"><h2>{t(UI.other)}</h2><div>{Object.entries(SERVICE_PAGES).filter(([key]) => key !== slug).slice(0, 4).map(([key, value]) => <Link key={key} href={`/services/${key}`}>{localizedService(key, value, lang).name}</Link>)}</div></section></div></main>;
}

export function LocationsHubView({ locations }) {
  const { lang, t } = useCopy();
  return <main className="seo-page"><div className="seo-page__inner"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">{t(UI.home)}</Link><span>/</span><span>{t(UI.locations)}</span></nav><header className="seo-hero"><p className="seo-eyebrow">{t(UI.local)}</p><h1>{t(UI.locationsTitle)}</h1><p>{t(UI.locationsIntro)}</p></header><section className="seo-card-grid">{locations.map(({ slug, page, loc }) => <article className="seo-card" key={slug}><h2><Link href={`/locations/${slug}`}>{lang === "es" ? page.labelEs : page.label}</Link></h2><p>{loc.full}</p><Link className="seo-text-link" href={`/locations/${slug}`}>{t(UI.hoursDirections)} →</Link></article>)}</section></div></main>;
}
